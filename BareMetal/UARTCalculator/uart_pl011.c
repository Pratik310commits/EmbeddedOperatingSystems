...
uart_error uart_configure(uart_config* config) {
    ...
    /* Disable the UART */
    uart0->CR &= ~CR_UARTEN;                                        /* ~UARTEnable */
    /* Finish any current transmission, and flush the FIFO */
    while (uart0->FR & FR_BUSY);                                    /* Flag Register and FR_BUSY*/
    uart0->LCRH &= ~LCRH_FEN;                                       /* Line control register &= ~ Line control register FIFO Enable*/
    ...
    if (config->parity) {
        lcrh |= LCRH_PEN;                           /* Line control register Parity Enable*/
        lcrh |= LCRH_EPS;                           /* Line control register Even parity select*/
        lcrh |= LCRH_SPS;                           /* Line control register Stick parity select*/
    } else {
        lcrh &= ~LCRH_PEN;
        lcrh &= ~LCRH_EPS;
        lcrh &= ~LCRH_SPS;
...
uart_error uart_getchar(char* c) {
    if (uart0->FR & FR_RXFE) {                      /* Receive FIFO empty */
        return UART_NO_DATA;
    }
